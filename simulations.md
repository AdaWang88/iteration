...
output[[1]] = rerun(100, sim_mean_sd((sample_size = n_list[[1]]))) %>% 
  bind_rows()
output[[2]] = rerun(100, sim_mean_sd((sample_size = n_list[[2]]))) %>% 
  bind_rows()

for(i in 1:4) {
  
  output[[i]] = rerun(100, sim_mean_sd((sample_size = n_list[[i]]))) %>% 
    bind_rows()
...
<img src="simulations_files/figure-gfm/unnamed-chunk-8-1.png" alt="" width="90%" />
